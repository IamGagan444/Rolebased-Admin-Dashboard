...
export const createUser = AsyncHandler(async (req, res,next) => {
  ...
  {
    return next(new ApiError(400,`${isEmptyFields.join(", ")} ${isEmptyFields.length>1?"are":"is"} required`))
  }
  ...
  if (
    ...
    return next(new ApiError(403, "you are not allowed to create this Role gg"));
  }
...
export const getAllUsers = AsyncHandler(async (req, res, next) => {
  ...
  return res
    .status(200)
    .json(new Apiresponse(201, users, "users data fetched successfully"));
});
